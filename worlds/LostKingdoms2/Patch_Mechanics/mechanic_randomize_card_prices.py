"""
Randomizes each card's shop price (buy and sell both read it).
...
Cards with a vanilla price of 0 are not purchasable and are left untouched,
so randomizing cannot make them appear in shops.
"""
...
def apply(patcher, output_data, minimum, maximum, mode, multiplier):
    rng = random.Random(output_data.get("Seed", -1) + 4)
