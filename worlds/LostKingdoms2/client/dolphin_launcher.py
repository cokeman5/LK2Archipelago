...
from .lostkingdoms2_settings import LostKingdoms2Settings, EmulatorSettings
...
class DolphinLauncher:
    """
    Manages interactions between the LK2 and the dolphin emulator.
    """
    ...
    def __init__(self, lostkingdoms2_settings: LostKingdoms2Settings = None):
        """
        ...
        :param auto_start: Determines if the the consumer should launch dolphin.
            Handled by the ArchipelagoLauncher in the host.yaml file.
        """
        if lostkingdoms2_settings is None:
            self.emulator_settings: EmulatorSettings = settings.get_settings().lostkingdoms2_options.dolphin_settings
        else:
            self.emulator_settings = lostkingdoms2_settings.dolphin_settings
